...
function travelingIndonesia(arr, emoney) {
  ...
  for(let data of arr) {
    ...
    let cityPassed = Math.abs(city.indexOf(departureCity) - city.indexOf(destinationCity));

    let baseCost = cost[transport] * cityPassed;
    let totalCost = baseCost - (baseCost * payment[emoney]);
